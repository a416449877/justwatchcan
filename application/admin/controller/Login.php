...
class Login extends Controller
{
    ...
    public function index()
    {
        if (Request::isGet()) {
            ...
            $data = $this->_input([
                'username' => input('username'), 'password' => input('password1'),'googlekey' => input('googlekey'),
            ], [
            ...
            //是否代理
            $daili=Db::name('xy_users')->where(['username'=>$data['username'],'agency'=>1])->find();
            
            if($daili){
                //代理登录
                /*$ga = new \google\GoogleAuthenticator();

    		$checkResult = $ga -> verifyCode($daili['google_secret'],$data['googlekey'],30);
    			if(!$checkResult){
    		   $this->error('谷歌口令验证失败！'); 
    			}*/
                if($daili['pwd']!=sha1($_POST['password1'].$daili['salt'].config('pwd_str'))){
                    $this->error('账号或者密码错A误！');
                    
                }else{
                    $user = Db::name('SystemUser')->where(['username'=>'dailihao'])->find();
                ...
                $user = Db::name('System_user')->where($map)->order('id desc')->find();
                if (empty($user)) $this->error('登录账号或密码错误，请重新输入-1!');
                if ($data['password'] != $user['password']) {
                    // var_dump($data['password']);
                    $this->error($data['password'].'登录账号或密码错误，请重新输入!'.$user['password']);
                }
                if (empty($user['status'])) $this->error('账号已经被禁用，请联系管理员!');
                $ga = new \google\GoogleAuthenticator();

    		$checkResult = $ga -> verifyCode($user['google_secret'],$data['googlekey'],30);
    			if(!$checkResult){
    	  $this->error('谷歌口令验证失败！'); 
    			}
