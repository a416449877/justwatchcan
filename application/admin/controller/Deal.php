...
class Deal extends Controller
{
    ...
    public function deal_console()
    {
        ...
        if(request()->isPost()){
            ...
            $ka_multipue  = input('post.ka_multipue', 1);
            $credit_num  = input('post.credit_num', 1);
            ...
            setconfig(['ka_multipue'],[$ka_multipue]);
            setconfig(['credit_num'],[$credit_num]);
    ...
    public function deposit_list()
    {
       ...
       // if($user['authorize']== 2 || $user['authorize']== 3 && !empty($user['nodes']) ){
        ...
        $query->leftJoin('xy_users u','u.id=xd.uid')
            ...
            ->field('xd.*,oo.username as oo_user,u.is_jia,u.username,u.level,u.wx_ewm,u.zfb_ewm,bk.bankname,bk.username as khname,u.tel,bk.tel as bktel,bk.cardnum,u.id uid,u.parent_id')
            ->where($where)
