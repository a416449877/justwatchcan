...
//        $this->fetch();
    ...
    public function edit_home_foot($id){
        $id = intval($id);
        if(request()->isPost()){
            $this->applyCsrfToken();
            $id      = input('post.id/d',0);
            ...
            $res = Db::table('xy_home_foot')->where('id',$id)->update(['image'=>$image,'title'=>$title,'content'=>$content]);
            if($res){
                $this->success('编辑成功','/admin.html#/admin/help/home_foot.html');
            }else
                $this->error('编辑失败');
        }

        $info = Db::table('xy_home_foot')->find($id);
        $this->assign('info',$info);
        $this->fetch();
    }
    ...
    public function add_home_foot(){
        if(request()->isPost()){
           ...
           // $url = input('post.url/s', '');
            ...
            $res = Db::table('xy_home_foot')->insert(['title'=>$title,'image'=>$image,'content'=>$content]);
            if($res){
                $this->success('提交成功','/admin.html#/admin/help/home_foot.html');
            }else
                $this->error('提交失败');
        }
        return $this->fetch();
