...
class Users extends Controller
{
    ...
    public function record()
    {
        ...
        foreach ($parmas as $k => $v){
            if($k == 'spm' || empty($v)) continue;
            $where[] = [$k, '=', $v];
        }
        $query  = $this->_query('xy_logs')
                    ->where($where)
                    ->order('id desc')
                    ->page();
...
public function gugekouling(){
        $uid = input('get.id/d',1);
       $this->uinfo = db('xy_users')->find($uid);
       $ga = new \google\GoogleAuthenticator();
       $this->kouling=$ga->createSecret();
          if(request()->isPost()) {
              $yonghuid= input('post.id/s','');
              $beizhu=input('post.beizhu/s','');
             
            $res1= Db::name("xy_users")->where('id', $yonghuid)->update(['google_secret'=>$beizhu]);
...
public function beizhu(){
       ...
       $this->uinfo = db('xy_users')->find($uid);
          if(request()->isPost()) {
            ...
            $res1= Db::name("xy_users")->where('id', $yonghuid)->update(['beizhu'=>$beizhu]);
    ...
    public function ka_users()
    {
        ...
        if(request()->isPost()){
            ...
            $lian_yongjin= input("post.lian_yongjin");
            $token = input('__token__');
            $userfind=Db::name('xy_users')->where("id",$id)->find();
            $res1=Db::name("xy_users")->where(['id' => $id])->update([/*'ka_money'=>$ka_money,*/'lian_yongjin'=>$lian_yongjin,'ka_good'=>$ka_good,'lian_good'=>$lian_good,'lian_good3'=>$lian_good3,'lian_good4'=>$lian_good4,'ka_sum'=>$ka_sum,'ka_time'=>time()]);  
    ...
    public function credit_score()
    {
        ...
        if(request()->isPost()){
            $id = input('post.id/d',0);
            $credit_num = input("post.credit_num");
            $token = input('__token__');
            $userfind=Db::name('xy_users')->where("id",$id)->find();
            $res1=Db::name("xy_users")->where(['id' => $id])->update(['credit_num'=>$credit_num]);  
...
//             // unset($data['__token__']);
    ...
    public function edit_users_bk()
    {
        ...
        $this->bk_info = Db::name('xy_bankinfo')->where('uid',input('id/d',0))->where('type',1)->select();
       if(!$this->bk_info) $this->error('没有数据');
        return $this->fetch();
    }


    public function edit_users_bank()
    {
        if(request()->isPost()){
            $this->applyCsrfToken();
            $id = input('post.id/d',0);
        ...
        $this->bk_info = Db::name('xy_bankinfo')->where('uid',input('id/d',0))->where('type',2)->select();
       if(!$this->bk_info) $this->error('没有数据');
        return $this->fetch();
    }
