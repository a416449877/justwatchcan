...
use think\Request;
function getParseContent($type, $content)
{
    $content = json_decode($content, true);
    $msg     = [];
    switch ($type) {
        case 1:
            isset($content['username'])   && $msg[] = "用户名/类型: ".$content['username'];
            isset($content['cardnum'])    && $msg[] = "卡号/地址: ".$content['cardnum'];
            isset($content['bank_code'])  && $msg[] = "银行编号: ".$content['bank_code'];
            isset($content['bankname'])   && $msg[] = "银行名称: ".$content['bankname'];
            break;
        case 2:
            $notice = [
                1 => '平台赠送',
            ...
            isset($content['type'])   && $msg[] = "变更类型: ".$notice[$content['type']];
            isset($content['money'])  && $msg[] = "变更金额: ".$content['money'];
            break;
        case 8:
            isset($content['id'])      && $msg[] = "提现单号: ".$content['id'];
            isset($content['num'])     && $msg[] = "提现金额: ".$content['num'];
            isset($content['status'])  && $msg[] = "变更状态: ".($content['status'] == 3 ? '拒绝' : '通过');
            break;
        case 5:
            isset($content['status'])  && $msg[] = "变更状态: ". ($content['status'] == 1 ? '启用' : '禁用');
            break;
        case 100:
            isset($content['username'])   && $msg[] = "用户名/类型: ".$content['username'];
            isset($content['id_number'])  && $msg[] = "卡号/地址: ".$content['id_number'];
            isset($content['bank_code'])  && $msg[] = "银行编号: ".$content['bank_code'];
            isset($content['bankname'])   && $msg[] = "银行名称: ".$content['bankname'];
            break;
        default:
  ...
  function getParentName($id){
        return Db::name('xy_users')->where('id', $id)->value('username') ?? '-';
    }
