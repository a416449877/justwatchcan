...
class Order extends Base
{
    ...
    public function do_order()
    {
        if(request()->isPost()){
           ...
           /*  if($userData['ka_sum'] > 0){
                     
                 $beginToday=mktime(0,0,0,date('m'),date('d'),date('Y'));
                 
                 $dans = Db::table('xy_convey')->where("uid = $uid ")->count();
                 
                 if($dans == $userData['ka_sum']){
                    
                     $where['addtime']=array('GT',$userData['ka_time']);
             
                    $rzSum = Db::table('xy_recharge')->where(['status'=>2,'uid'=>$uid])->where($where)->sum('num');
                     if($rzSum < $userData['ka_money']){
                         return json(['code'=>1,'info'=>lang('您需要充值').$userData['ka_money'].lang('才能抢单')]);
                     }
                 }
