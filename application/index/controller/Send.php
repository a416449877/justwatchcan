...
class Send extends Controller
{
    ...
    public function sendsms(Request $request)
    {
        ...
        $code=substr($code,1);
        $result = $this->smshong($tel,$num,$code);
    ...
    //天一宏
    public function smshong($tel,$code,$g_code){
        date_default_timezone_set("PRC");
        $name="cs_dook1y";
        $password="VtQsBo7f";
        $time=date("Y",time()).date("m",time()).date("d",time()).date("H",time()).date("i",time()).date("s",time());
        $data["account"]=$name;
        $data["sign"]=md5($name.$password.$time);
        $data['datetime']=$time;
        $data["numbers"]=$g_code.$tel;
        $data["content"]=urlencode("[uBey App] Your verification code is ".$code);
        $data=http_build_query($data);
        $url="http://sms.skylinelabs.cc:20003/sendsmsV2?".$data;
        $ch = curl_init();
    ...
    public function smsbao($tel,$code,$g_code)
    {
        ...
        $data=[];
        $data['appkey']='U39iVomw';
        $data['secretkey']='vHXaJHXK';
