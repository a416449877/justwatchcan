...
class Crontab extends Controller
{
    public function lang_text(){
        ...
        define("APP_ID",         "20210103000662228"); //替换为您的APPID
        define("SEC_KEY",        "XVN8rZ6Iy3N6f2TlKIXH");//替换为您的密钥
        
        $good_list=Db::table('xy_goods_list')->where('id','egt',576)->select();
        foreach ($good_list as $value){

            $fy_shop_name=translate($value['shop_name'],'zh','en');
            $fy_goods_name=translate($value['goods_name'],'zh','en');
            
            $data_sql=[
                'shop_name'=>$fy_shop_name['trans_result'][0]['dst']?$fy_shop_name['trans_result'][0]['dst']:$value['shop_name'],
                'goods_name'=>$fy_goods_name['trans_result'][0]['dst']?$fy_goods_name['trans_result'][0]['dst']:$value['goods_name'],
            ];
            
            if($value['goods_info']){
                $fy_goods_info=translate($value['goods_info'],'zh','en');
                
                $data_sql['goods_info']=$fy_goods_info['trans_result'][0]['dst']?$fy_goods_info['trans_result'][0]['dst']:$value['goods_info'];
            }
            

            Db::table('xy_goods_list')->where(['id'=>$value['id']])->update($data_sql);
            echo $value['id'].'<br>';
        }
